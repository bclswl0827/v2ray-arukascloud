...
RUN apk add --no-cache --virtual .build-deps ca-certificates curl wget \
 ...
 && cd /caddy \
 && wget https://github.com/mholt/caddy/releases/download/v$CADDYVER/caddy_v$CADDYVER_linux_amd64.tar.gz \
 && tar -xvf /caddy/caddy_v$CADDYVER_linux_amd64.tar.gz -C /caddy \
 && chmod +x /caddy/caddy \
